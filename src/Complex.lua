...
type Complex = _t.Complex; local Complex = {} do
    local ComplexClass = {__metatable = "Locked"}
    ...
    function Complex.fromPolar(pa: number, r: number): Complex
        return new(math.cos(pa)*r, math.sin(pa)*r) :: any
    end
    ...
    function ComplexClass.Orbit(z: Complex, c: Complex, maxIter: number, escapeOrbit: number?): number
        ...
        local iter: number = 0
        -- optimized iteration, only 3 multiplications needed per iteration
